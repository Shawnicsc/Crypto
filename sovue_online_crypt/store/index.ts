...
export async function encryptText(plainText: string, algrithm: { name: string, value: string }): Promise<string> {
  const alg = algrithm.name;
  console.log({ alg , plainText });
  const response = await axios.post('/api/encrypt', { alg , plainText });
  return response.data.encryptedText;
}
export async function decryptText(plainText: string, algrithm: { name: string, value: string }): Promise<string> {
  const alg = algrithm.name;
  const response = await axios.post('/api/decrypt', { alg , plainText });
  return response.data.decryptedText;
}
axios.defaults.baseURL = 'http://localhost:9002'; // �޸�Ϊ��ĺ�� API ��ַ
